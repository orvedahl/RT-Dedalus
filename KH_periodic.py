...
tanh_width = 0.025
tanh_center = 0.25
phi = 0.5*(1-np.tanh((z-tanh_center)/tanh_width)*np.tanh((z-0.5-tanh_center)/tanh_width))-0.5

A_u = 2000

T['g'] = phi + A0*np.sin(2*z/Lz)*np.random.randn(*T['g'].shape)
u['g'] = A_u*phi
w['g'] = A_u*1e-2*np.sin(2*z/Lz)*np.cos(4*np.pi*x/Lx)
